...
def train_mle_estimator(train_images, train_labels):
    ...
    N = len(train_labels)
    C = train_labels.sum(axis=0).reshape(-1, 1)
    pi_mle = (C/float(N)).T
    theta_mle = ((train_labels.T @ train_images) / C).T
...
def train_map_estimator(train_images, train_labels):
    ...
    C = train_labels.sum(axis=0).reshape(-1, 1)
    pi_map = (C/float(N)).T
    theta_map = (((train_labels.T @ train_images) + 2) / (C + 4)).T
...
def log_likelihood(images, theta, pi):
    ...
    a = np.log(pi) + images @ np.log(theta) + (1 - images) @ np.log(1 - theta)
    b = np.log((np.exp(a)).sum(axis=1)).reshape((-1, 1))
    log_like = a - b
    return log_like
